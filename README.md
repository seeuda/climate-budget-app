...
- 🗂️ **多層次複選導引**：計畫類別可複選最多 3 項，細項分類依已選類別聯合顯示並支援複選，工項整合所有已選細項聯集
- 💰 **預算拆解防呆**：即時顯示剩餘預算、鎖定超額送出
- 📤 **雙格式匯出**：JSON（系統串接，含 `categories[]` / `sub_categories[]` 陣列）與 CSV（Excel 分析）
- 🚦 **風險分級警示**：四色燈號自動判定（300萬/1000萬/2000萬/1億）
- 🛡️ **工項保護機制**：補選/取消類別或細項時不清空既有工項，僅在進入下一步時移除已失效工項
...
步驟一：輸入計畫基本資訊
    ↓ （關鍵字自動偵測，右側即時顯示建議工項）
步驟二：複選計畫類別（最多 3 項）+ 複選細項分類
    ↓ 類別支援複選，細項依已選類別聯合顯示
    ↓ 補選/取消類別時，既有工項勾選不會被清空
步驟三：勾選氣候相關工項
    ↓ 依所有已選細項聯集顯示，重複工項自動去重
    ↓ 超出範圍的舊工項會提示，按「下一步」才移除
步驟四：填寫各工項氣候預算金額
    ↓ 剩餘預算即時顯示，超額時鎖定下一步
...
> **JSON 結構異動說明（v1.0 複選版）**  
> `climate_assessment` 欄位由單值改為陣列：
> - `categories: []` — 已選計畫類別 ID 陣列  
> - `category_labels` — 中文標籤字串（逗號分隔，供閱讀）  
> - `sub_categories: []` — 已選細項分類 ID 陣列  
> - `sub_category_labels` — 中文標籤字串
