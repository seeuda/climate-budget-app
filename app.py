...
def init_state():
    ...
    defaults: dict = {
        ...
        "max_unlocked_step":        0,
        "scroll_to_top":            False,
        "step0_case_name_input":    "",
        "step0_dept_select":        "（請選擇）",
        "step0_dept_other_input":   "",
        "step0_budget_input":       "",
        "step0_manual_override_input": False,
        # ── 案件基本資訊 ──────────────────────────────────────────────
        "case_name":                "",
        "dept":                     "",
        "agency_name":              "",
...
def sync_step0_draft_to_state():
    """將 Step0 尚未按「下一步」的草稿輸入同步回主狀態（供快速跳頁檢核使用）。"""
    if st.session_state.step != 0 or not st.session_state.use_manual_case_input:
        return

    case_name = str(st.session_state.get("step0_case_name_input", "")).strip()
    dept = str(st.session_state.get("step0_dept_select", "（請選擇）")).strip()
    dept_other = str(st.session_state.get("step0_dept_other_input", "")).strip() if dept == "其他" else ""
    budget_input = str(st.session_state.get("step0_budget_input", "")).strip()
    ...
    manual_override = bool(st.session_state.get("step0_manual_override_input", False)) if below_threshold else False

    st.session_state.case_name = case_name
    st.session_state.dept_other = dept_other
    st.session_state.dept = selected_dept if selected_dept not in ("", "（請選擇）") else ""
    st.session_state.budget = budget_val
    st.session_state.manual_override = manual_override
    st.session_state.kw_matches = detect_keywords(case_name)
...
if st.session_state.step == 0:
    ...
    with col1:
        ...
        if use_manual_case_input:
            ...
            case_name = st.text_input(
                "📌 標案名稱",
                value=st.session_state.step0_case_name_input or st.session_state.case_name,
                placeholder="例：彰化縣○○公園綠美化工程",
                help="請輸入公文中的完整標案名稱，系統將自動偵測氣候關鍵字",
                key="step0_case_name_input",
            )

            departments = get_department_options()
            dept_options = ["（請選擇）"] + departments + ["其他"]
            dept_index = 0
            current_dept = st.session_state.step0_dept_select
            if current_dept in departments:
                dept_index = dept_options.index(current_dept)
            elif current_dept and current_dept not in ("（請選擇）", ""):
                dept_index = dept_options.index("其他")
            elif st.session_state.dept in departments:
            ...
            if dept == "其他":
                dept_other = st.text_input(
                    "請填寫主辦局處名稱",
                    value=st.session_state.step0_dept_other_input or st.session_state.dept_other,
                    placeholder="例：文化局",
                    key="step0_dept_other_input",
                ).strip()

            budget_input = st.text_input(
                "💰 決標金額（元）",
                value=st.session_state.step0_budget_input or (str(int(st.session_state.budget)) if st.session_state.budget else ""),
                placeholder="例：15000000",
                help="請輸入決標金額（純數字，不含逗號）",
                key="step0_budget_input",
            )
        else:
    ...
    if below_threshold:
        ...
        manual_override = st.checkbox(
            UI["manual_override_label"],
            value=st.session_state.manual_override,
            key="step0_manual_override_input",
        )
    else:
        manual_override = False
        st.session_state.step0_manual_override_input = False
