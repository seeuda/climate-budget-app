...
def generate_export_json(state):
    ...
    result = {
        ...
        "assessment_metadata": {
            ...
            "overflow_category_note": state.get("overflow_category_note", ""),
...
if st.session_state.step == 0:
    ...
    with st.expander("📋 哪些案件通常不需納入評估？（點擊展開）"):
        st.markdown("**❌ 以下類型通常明確不需評估：**")
        clear_exclusions = [
        ...
        for g in clear_exclusions:
            st.markdown(f"• {g}")
        st.markdown("---")
        st.markdown("**⚠️ 以下類型需要再判斷，不一定排除：**")
        st.markdown(
            "• **節慶／藝文活動**："
            "若屬一般觀光節慶（如桐花祭、演唱會）→ 不需評估；"
            "若屬**氣候變遷宣導、淨零教育推廣活動** → 請繼續填報（進入步驟二選「公正轉型與社會韌性」類別）"
        )
        st.markdown(
            "• **景觀美化工程**："
            "若無降溫、碳匯、透水鋪面等明確氣候目標 → 不需評估；"
            "若有上述目標 → 請繼續，並在步驟二選擇「公園／綠美化／生態環境」類別"
        )
        st.markdown(
            "• **補助計畫**："
            "若補助對象與氣候完全無關（如急難救助） → 不需評估；"
            "若補助民眾進行節能、低碳、防災等改善 → 請繼續"
        )
...
# ═══════════════════════════════════════════════════════════════════
    ...
    def refine_quick_guide_cats(
        raw_cats: list[str],
        keyword_cats: list[str],
        selected_cats: list[str],
        max_count: int = 3,
    ) -> list[str]:
        """縮減快速導引建議，並優先推薦尚未勾選的類別。"""
        ...
        if len(deduped) <= max_count:
            ...
            ordered = (prioritized + tail)[:max_count]

        unselected = [cat for cat in ordered if cat not in selected_cats]
    ...
    QUICK_GUIDE_ENTRIES = [
        {
            "key": "qg_advocacy",
            "label": "🎯 主題性長期宣導／產業與特定群體輔導",
            "hint": "依業務主軸或持續性的主題推廣與輔導（例如長期推廣、協助特定服務對象提高社會韌性的"
                    "輔導資源），可歸入「公正轉型與社會韌性」（G）；若同時包含前期規劃、診斷評估或顧問設計，"
                    "可加選「評估／規劃類」（F）。",
            "cats": ["G", "F"],
            "subs": ["G2", "F1"],
        },
        {
            "key": "qg_training",
            "label": "📚 系統性課程培訓／社區能力建構（多堂次）",
            "hint": "偏向有培訓目標與學習歷程的系列課程、研習或社區培力（例如多堂次培訓、種子講師養成），通常歸入「公正轉型與社會韌性」"
                    "（G）中的「對特定服務對象的宣導、教育活動、推廣課程」細項（G2）。",
            "cats": ["G"],
            "subs": ["G2"],
        },
        {
    ...
    if quick_guide_cats:
        remaining_quick_guide_cats = [cat for cat in quick_guide_cats if cat not in st.session_state.selected_categories]
        readable_quick_guide_cats = "、".join([
            f"{taxonomy_label_map.get(cat_id, cat_id)}（{cat_id}）"
            for cat_id in remaining_quick_guide_cats
        ])
        if readable_quick_guide_cats:
            st.caption(f"📌 快速導引目前建議您優先勾選：{readable_quick_guide_cats}。")
        else:
            st.caption("✅ 快速導引建議類別您都已勾選，可直接往下選細項。")
    ...
    for i, cat in enumerate(taxonomy):
        ...
        is_quick_guide = cat["id"] in quick_guide_cats

        with (col1 if i % 2 == 0 else col2):
            badge_parts = []
            if is_suggested:
                badge_parts.append("⭐ 關鍵字建議")
            if is_quick_guide:
                badge_parts.append("📌 快速導引建議")
...
**{alert['badge']} 計畫規模（隱含碳潛力）：** {alert['label']}
    ...
    raw_user_note = st.session_state.get("user_note", "").strip()
    overflow_note = st.session_state.get("overflow_category_note", "").strip()
    overflow_note_tagged = f"【超出三類補充脈絡】{overflow_note}" if overflow_note else ""
    export_user_note = raw_user_note
    if overflow_note_tagged and overflow_note_tagged not in export_user_note:
        export_user_note = f"{export_user_note}\n{overflow_note_tagged}".strip() if export_user_note else overflow_note_tagged
        if raw_user_note:
            st.caption("ℹ️ 匯出時將自動附加「超出三類補充脈絡」於補充說明。")
    ...
    for ib in state.item_budgets:
        ...
        rows.append({
            ...
            "氣候工項"          : ib["label"],
            "計畫補充敘述"      : state.get("work_item_description", ""),
            "工項金額"          : ib["amount"],
            "工項佔總預算%"     : item_ratio,
